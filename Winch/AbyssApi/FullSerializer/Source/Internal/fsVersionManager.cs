...
namespace FullSerializer.Internal {
    internal static class fsVersionManager {
        ...
        private static void VerifyConstructors(fsVersionedType type) {
            ConstructorInfo[] internalConstructors = type.ModelType.GetDeclaredConstructors();

            for (int i = 0; i < type.Ancestors.Length; ++i) {
                Type requiredConstructorType = type.Ancestors[i].ModelType;

                bool found = false;
                for (int j = 0; j < internalConstructors.Length; ++j) {
                    var parameters = internalConstructors[j].GetParameters();

...
namespace FullSerializer {
    ...
    internal class fsAotCompilationManager {
        ...
        internal static Dictionary<Type, string> AvailableAotCompilations {
            get {
                for (int i = 0; i < _uncomputedAotCompilations.Count; ++i) {
                    ...
                    _computedAotCompilations[item.Type] = GenerateDirectConverterForTypeInCSharp(item.Type, item.Members, item.IsConstructorinternal);
                }
        ...
        private struct AotCompilation {
            ...
            internal bool IsConstructorinternal;
        }
        ...
        internal static bool TryToPerformAotCompilation(fsConfig config, Type type, out string aotCompiledClassInCSharp) {
        ...
        /// <param name="isConstructorinternal">True if the constructor is internal.</param>
        internal static void AddAotCompilation(Type type, fsMetaProperty[] members, bool isConstructorinternal) {
            _uncomputedAotCompilations.Add(new AotCompilation {
                Type = type,
                Members = members,
                IsConstructorinternal = isConstructorinternal
            });
        }
        ...
        /// </summary>
        private static string GenerateDirectConverterForTypeInCSharp(Type type, fsMetaProperty[] members, bool isConstructorinternal) {
            ...
            if (isConstructorinternal) {
                sb.AppendLine("            return new " + typeName + "();");
            }
            else {
                sb.AppendLine("            return Activator.CreateInstance(typeof(" + typeName + "), /*noninternal:*/true);");
            }
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine("}");
